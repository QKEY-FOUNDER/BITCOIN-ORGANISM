...
width = 1.2 + accel * 4
ax.plot(curve_x[i:i+2], curve_y[i:i+2], color=(0.2, 0.7, 1.0, alpha), linewidth=width)
...
ax.contourf(xx, yy, density, levels=6, cmap=“Blues”, alpha=0.10)
except Exception:
return
...
ax.plot([x1, x2], [y1, y2], color=(0.6, 0.8, 1.0, 0.10 + strength * 0.2), linewidth=1 + strength * 2)
...
def project_future_path(history, steps=8):
if len(history) < 3:
return []
points = np.array(history)
...
return
speed = st.slider(“Base speed”, 0.01, 0.25, 0.035)
...
dx = cx - x
dy = cy - y
dist = np.sqrt(dx2 + dy2) + 1e-5
x += dx / dist * 0.01
y += dy / dist * 0.01
ax.scatter(cx, cy, s=140, color=(1, 0.8, 0.2, 0.8), zorder=6)
...
ax.contourf(xx, yy, density, levels=8, alpha=0.06)
draw_ghost_field(ax, history, xmin, xmax, ymin, ymax)
draw_gravity_links(ax, history)
draw_comet_trail(ax, prev2[“tension”], prev2[“pressure”], prev1[“tension”], prev1[“pressure”], x, y, accel)
target_angle = i * (0.2 + accel * 2)
angle = prev_angle + (target_angle - prev_angle) * 0.12
prev_angle = angle
r1 = 0.02 + accel * 0.03
r2 = 0.04 + accel * 0.05
size_base = 80 + accel * 200
ax.scatter(x + r2np.cos(angle), y + r2np.sin(angle), s=size_base, color=(0.2,0.4,1,0.9))
ax.scatter(x + r1np.cos(angle+np.pi), y + r1np.sin(angle+np.pi), s=size_base0.6, color=(0.3,0.5,1,0.7))
ax.scatter(x + r2np.cos(angle+np.pi/2), y + r2np.sin(angle+np.pi/2), s=size_base0.6, color=(1,0.3,0.3,0.8))
ax.scatter(x + r1np.cos(angle-np.pi/2), y + r1np.sin(angle-np.pi/2), s=size_base0.4, color=(1,0.4,0.4,0.6))
ax.scatter(x, y, s=120 + accel300, color=(0.6,0,0.8,1), zorder=5)
future = project_future_path(history)
if len(future) > 0:
ax.plot(future[:, 0], future[:, 1], linestyle=“dashed”, linewidth=2, color=(1.0, 0.5, 0.2, 0.5))
history.append((x, y))
if len(history) > 6:
history.pop(0)
dynamic_speed = max(0.02, min(speed * (1.2 - accel*3), 0.2))
